...
// Danh sách các thư mục LAB
Route::get('/baitap', function () {

    $path = public_path('LabTH');

    if (!is_dir($path)) {
        return "Thư mục LabTH chưa tồn tại trong public/.";
    }

    // Lấy danh sách thư mục bên trong LabTH
    $labs = array_filter(glob($path . '/*'), 'is_dir');

    // Chỉ lấy tên folder
    $labs = array_map('basename', $labs);

    return view('baitap.index', compact('labs'));
});

// Danh sách file trong 1 LAB
Route::get('/baitap/{lab}', function ($lab) {

    $path = public_path("LabTH/$lab");

    if (!is_dir($path)) {
        abort(404);
    }

    // Lấy file và folder bên trong
    $files = array_map('basename', glob("$path/*"));

    return view('baitap.files', compact('lab', 'files'));
});
